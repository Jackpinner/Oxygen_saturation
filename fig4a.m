...
ir_filter  = data{:, 2};  % 第2列: LED 交流信号
red_filter = data{:, 3};  % 第3列: VCSEL 交流信号
...
%% 2. 绘制图表 5(a)
figure('Name', 'Fig 5(a): 660LED与VCSEL信号完整对比', 'Position', [100, 200, 800, 400]);

% --- 核心视觉呈现：粗蓝底 + 细黄虚线 ---

% 1. 绘制底部的 LED 波形 
% 使用较粗的线条 (LineWidth = 3) 和经典的 MATLAB 蓝色
p1 = plot(t, ir_filter, '-', 'Color', [0.4940, 0.1840, 0.5560], 'LineWidth', 3); 
hold on;

% 2. 绘制顶部的 VCSEL 波形 
% 使用细虚线 (LineWidth = 1.5) 和醒目的学术明黄色
% 这样蓝色底会从虚线的空隙中透出来，极其直观地展现高度重合
p2 = plot(t, red_filter, '-', 'Color', [0.6350, 0.0780, 0.1840], 'LineWidth', 3); 
...
% 图例设置
legend([p1, p2], {'LED Signal ', 'VCSEL Signal '}, 'Location', 'northeast');
